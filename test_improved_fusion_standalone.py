#!/usr/bin/env python3
"""
独立测试改进后的自适应特征融合模块（不依赖完整的 diffusers 库）
...
class AdaptiveFeatureFusion(nn.Module):
    """
    自适应特征融合模块 (Adaptive Feature Fusion Module) - 改进版

    改进点：
    1. 添加残差连接：保留原始特征，避免信息丢失
    2. 零初始化：融合模块初始时不影响预训练模型，逐步学习
    3. 可学习的融合强度：通过 alpha 参数控制融合程度
    """
    def __init__(self, channels, reduction_ratio=16, use_residual=True, init_alpha=0.0):
        super().__init__()
        ...
        # 零初始化
        self._zero_init_attention_modules()

    def _zero_init_attention_modules(self):
        """零初始化注意力模块的最后一层"""
        nn.init.zeros_(self.channel_attention[3].weight)
    ...
    def forward(self, x):
        ...
        if self.use_residual:
            out = identity + self.alpha * (x_spatial - identity)
        else:
            out = x_spatial
...
class LightweightAdaptiveFusion(nn.Module):
    """轻量级自适应特征融合模块"""
    def __init__(self, channels, reduction_ratio=32, init_alpha=0.0):
        super().__init__()

        # 简化的通道注意力
        self.channel_attention = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(channels, max(channels // reduction_ratio, 8), 1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(max(channels // reduction_ratio, 8), channels, 1, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        identity = x
        channel_weight = self.channel_attention(x)
        x_attention = x * channel_weight
        out = identity + self.alpha * (x_attention - identity)
        return out
