...
from examples.freeinpaint.utils.ptp_utils import AttendExciteAttnProcessor, AttentionStore
from examples.freeinpaint.utils.attn_utils import fn_show_attention, fn_smoothing_func, fn_get_topk, fn_clean_mask, fn_get_otsu_mask
...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        diffusers.     use_safetensors=True,
        diffusers. )
        >>> pipe.to("cuda")
        ...
        >>> image = pipe(
        diffusers.     prompt=prompt, image=init_image, mask_image=mask_image, num_inference_steps=50, strength=0.80
        diffusers. ).images[0]
        ```
"""
...
class StableDiffusionXLInpaintOptNoGuidancePipeline(
    ...
    def cond_fn(
        ...
        harmonic_score, _ = self.harmonic_reward(image*mask+cond_image*(1-mask), mask)
    ...
    def fn_initno(
        ...
        # optimizer = Adam([log_var, mu], lr=initno_lr, eps=1e-4)
        ...
        for iteration in tqdm(range(max_step)):
            ...
            for i, t in enumerate(timesteps):
                ...
                if use_grad_checkpoint:
                    noise_pred_text = checkpoint.checkpoint(
                        self.unet, 
                        latent_model_input,
                        t, 
                ...
                else: noise_pred_text = self.unet(
                    latent_model_input,
                    t, 
                    encoder_hidden_states=text_embeddings[1].unsqueeze(0),
                    timestep_cond=None,
                ...
                if denoising_step_for_loss > 1:
                    with torch.no_grad():
                        if use_grad_checkpoint:
                            noise_pred_uncond = checkpoint.checkpoint(
                                self.unet, 
                                latent_model_input,
                                t, 
                                encoder_hidden_states=text_embeddings[1].unsqueeze(0),
                        ...
                        else: noise_pred_uncond = self.unet(
                                latent_model_input,
                                t, 
                                encoder_hidden_states=text_embeddings[1].unsqueeze(0),
                                timestep_cond=None,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        mask_condition = mask.clone().detach()
        ...
        # 10.2  opt noise
        attn_res = int(np.ceil(width/self.attn_res_scale_factor)), int(np.ceil(height/self.attn_res_scale_factor))
        ...
        run_initno = True
        added_cond_kwargs = {"text_embeds": add_text_embeds.chunk(2)[1] if self.do_classifier_free_guidance else add_text_embeds, 
                             "time_ids": add_time_ids.chunk(2)[1] if self.do_classifier_free_guidance else add_time_ids}
        if ip_adapter_image is not None or ip_adapter_image_embeds is not None:
            added_cond_kwargs["image_embeds"] = image_embeds.chunk(2)[1] if self.do_classifier_free_guidance else image_embeds
            ...
            with torch.enable_grad():
                ...
                for round in range(max_round):
                    optimized_latents, optimization_succeed, loss = self.fn_initno(
                        latents,
                        mask.chunk(2)[1] if self.do_classifier_free_guidance else mask,
                        masked_image_latents.chunk(2)[1] if self.do_classifier_free_guidance else masked_image_latents,
                        prompt_embeds,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                cond_image_guidance = init_image.clone().detach().to(dtype=latents.dtype, device=device)
                cond_image_guidance = ((cond_image_guidance + 1) / 2).clamp(0, 1)
                ...
                added_cond_kwargs_guidance = {"text_embeds": add_text_embeds.chunk(2)[1] if self.do_classifier_free_guidance else add_text_embeds,
                                              "time_ids": add_time_ids.chunk(2)[1] if self.do_classifier_free_guidance else add_time_ids}
                if ip_adapter_image is not None or ip_adapter_image_embeds is not None:
                    added_cond_kwargs_guidance["image_embeds"] = image_embeds.chunk(2)[1] if self.do_classifier_free_guidance else image_embeds
        ...
        # apply watermark if available
        if self.watermark is not None:
            final_images = self.watermark.apply_watermark(final_images)
        ...
        if padding_mask_crop is not None:
            final_images = [self.image_processor.apply_overlay(mask_image, original_image, i, crops_coords) for i in final_images]
        ...
        return StableDiffusionXLPipelineOutput(images=final_images)