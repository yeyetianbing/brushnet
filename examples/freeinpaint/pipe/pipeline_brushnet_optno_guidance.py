...
from diffusers.models import AutoencoderKL, BrushNetModel, ImageProjection, UNet2DConditionModel
...
class StableDiffusionBrushNetOptNoGuidancePipeline(
    DiffusionPipeline,
    ...
    def _encode_prompt(
        ...
        **kwargs,
        ...
        prompt_embeds_tuple, _ = self.encode_prompt(
            prompt=prompt,
            device=device,
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            if hasattr(self.text_encoder.config, "use_attention_mask") and self.text_encoder.config.use_attention_mask:
                ...
                attention_mask = None
            attention_mask_cond = text_inputs.attention_mask.to(device)
        ...
        return prompt_embeds, negative_prompt_embeds, attention_mask_cond
    ...
    def encode_image(self, image, device, num_images_per_prompt, output_hidden_states=None):
        ...
        if output_hidden_states:
            ...
            image_enc_hidden_states = image_enc_hidden_states.repeat_interleave(num_images_per_prompt, dim=0)   #[1,257,1280]
        ...
        for iteration in range(max_step):# tqdm(range(max_step)):
            ...
            for i, t in enumerate(timesteps):
                ...
                down_block_res_samples, mid_block_res_sample, up_block_res_samples = self.brushnet(
                    ...
                    brushnet_cond=condition_latents,
                    conditioning_scale=cond_scale, 
                    guess_mode=False, 
                    return_dict=False
                    )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        prompt_embeds, negative_prompt_embeds, attention_mask = self.encode_prompt(
            prompt,
            device,
            num_images_per_prompt,
        ...
        if isinstance(brushnet, BrushNetModel):
            ...
            original_mask=(original_mask.sum(1)[:,None,:,:] < 0).to(image.dtype)    # [2,1,512,512], 0 indicates masked area to inpaint
        ...
        mask = torch.nn.functional.interpolate(
                    original_mask.to(torch.float16), 
                    size=(
                        conditioning_latents.shape[-2], 
                        conditioning_latents.shape[-1]
                    )
                ).to(conditioning_latents.dtype)   # [2,1,64,64]
        ...
        run_initno = True
        if run_initno and self.opt_noise_steps > 0 and self.max_round_initno > 0:
            ...
            with torch.enable_grad():
                ...
                for round in range(max_round):
                    ...
                    optimized_latents, optimization_succeed, loss = self.fn_initno(
                        latents,
                        1-original_mask.chunk(2)[1] if self.do_classifier_free_guidance else 1-original_mask,
                        prompt_embeds,
                        attention_mask,
                        conditioning_latents.chunk(2)[1] if self.do_classifier_free_guidance else conditioning_latents,
                        cond_scale,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)    # this is a identical function
                ...
                # perform reward guidance
                text_embeddings_guidance = (
                    prompt_embeds.chunk(2)[1] if self.do_classifier_free_guidance else prompt_embeds
                )
                cond_image_guidance = image.chunk(2)[1] if self.do_classifier_free_guidance else image
        ...
        if not output_type == "latent":
            image_pred = self.vae.decode(latents / self.vae.config.scaling_factor, return_dict=False, generator=generator)[0]
        ...
        # 3. Blending Logic
        mask_np = original_mask.chunk(2)[1].cpu().numpy() if self.do_classifier_free_guidance else original_mask.cpu().numpy()
